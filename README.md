...
没有 WebGL2，或者拿不到浮点渲染目标时，退回 Canvas 2D（`?renderer=2d` 可以强制）。两条路径跑同一套 182 项检查：渲染组整体跑两遍，再加一次 `--disable-webgl` 的强制回退。回退路径做不到的事在上面写明了。
...
| | 服务器版 | 单文件版 | 在线版 |
|---|---|---|---|
| 入口 | `server.js` + `public/` | `oscilloscope-standalone.html` | GitHub Pages |
| 启动 | `node server.js` | 双击打开 | [打开](https://cht-1192.github.io/web-oscilloscope-music-player-visualizer/) |
| 音频 | 自动列出项目目录，也支持拖拽和选择 | 拖拽或选择文件 | 拖拽或选择文件 |
| 适合 | 日常用、局域网共享、大文件流式播放 | 拷给别人、离线演示 | 发个链接给别人看一眼 |

在线版是每次推送到 `main` 由 Actions 构建发布的（`.github/workflows/pages.yml`），根目录是模块版，`/standalone.html` 是同一批源码拼出来的单文件版。它没有服务端，所以 `/api/tracks` 不存在，进来时播放列表是空的，音频得自己拖进去；只想看画面就加 `?demo=1`。
...
node test/verify.js        # 182 项端到端验证
...
列表顶部一个过滤框加一个排序框：过滤按名字、格式、编码和那行 meta 匹配，所以输 `flac` 或 `44.1` 都能筛；排序按名称、时长、采样率、大小，旁边那个箭头切升降。顺序是真的重排数组，正在播放的那首按身份找回自己的位置，不会因为重排而串台。

底部的模式按钮（快捷键 `M`）在顺序、单曲循环、随机之间轮换，图标三态，非顺序时按钮变色。一首放完按模式走：单曲重放同一首，随机换一首别的，顺序接下一首；只有一首时顺序模式不循环，想循环就用单曲。

每行右边的 `✕` 把这首从列表里拿掉，正在播放的那首被拿掉就会停下来并卸载，磁盘上的文件不动（服务器扫出来的歌刷新后会重新出现，这点写在按钮的 title 里）。底部的清空要按两下，第二下才真的清。

播放位置和模式记在本地：下次打开会回到同一首、同一个位置，但是暂停的。一个页面自己开始出声是会被关掉的，按 `空格` 或加 `?play=1` 才会放。拖进来的本地文件不参与这件事，它的 `blob:` 地址关掉标签页就没了。

## 两个只在一条路径上有效的滑块

残留修的是 8 位路径的量化地板。余辉拉高会让残影更亮而不是留得更久，这个滑块控制定期擦除的力度。能量路径没有地板可擦，所以那一行在 WebGL 下置灰；测试也量了这件事，滑块拉到 100% 和 0%，结果必须一致。

光晕是 halation 云，只在能量路径上有。取样用四个尺度（2 / 16 / 64 / 256 px 的格子），三个粗的各自先做一次 13 点模糊再取，否则纹素格子会被放大，细笔画看着像一串小点；64 和 256 那两级还过一条 0.75 次幂，因为它们取的是格子里的平均值，而平均值会低估 1/r 的长尾，不开这一下云只有两三级灰，滑块拖到头也看不出来。它是屏幕亮度的卷积，所以在密集图形周围是一团包住整体的雾，单根细线周围只是淡淡一圈。光晕为 0 时整条路径一行代码都不跑。
...
test/sections/*.js              182 项按失败方式分段：port、http、render-*、presets、playlist、resample、standalone
test/bench.js                   性能基准
```
