...
node test/verify.js        # 159 项端到端验证；--only=port|http|render|presets|resample|standalone 可只跑一段
...
1. **没有回扫线。** 不 `closePath`，不跨帧连path，跨帧/回扫用显式阈值丢掉（比平均束速快 `blankRatio` 倍的线段直接不画，默认 10×，范围 1–15× 半档）。
...
**halation 云**（`S.halo`，默认 0，只在 WebGL 路径）：加在色调映射之后，输入是有界的已发光亮度。三个尺度取样：mip1（权重 0.40）、mip4 与 mip6（各先做一次 13 点 tent 模糊，权重 0.33 / 0.27，不模糊就会把纹素格子放大成"一串小点"）。幅度 `0.9·(halo/100)^1.5`，硬上限由 `min(1, L + mix·h)` 保证。`halo = 0` 时整个 pass 一次都不跑，画面与旧版逐像素一致。因为它是屏幕亮度的卷积，密集图形周围明显、单根细线周围几乎看不见，这是它的性质不是缺陷。
...
- 光晕径向剖面（合成细线）：`10/18/28/40/55/75/100/130 px → 12/9/5/3/1/1/0/0`，单调、无断崖；光晕 0 时 10 px 与 28 px 处严格为 0。
